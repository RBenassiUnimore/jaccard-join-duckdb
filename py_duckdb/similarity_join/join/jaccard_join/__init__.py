...
class _JaccardTemplateJoin(ABC):
    ...
    def tokenize(self):
        self._con.execute(f"drop view if exists {TOKENS_VIEW}").execute(
            f"create view {TOKENS_VIEW} as " + self._tokenizer.query()
        )
...
class _JaccardSelfJoin(_JaccardTemplateJoin):
    ...
    def build_input(self):
        self._con.execute(f"drop view if exists {INPUT_TABLE}").execute(
            f"create view {INPUT_TABLE} as "
            f"select '{self._l_table}' as src, {self._l_key_attr} as rid, {self._l_join_attr} as val "
            f"from '{self._l_table}' "
        )

    def document_frequency(self):
        self._con.execute(f"drop view if exists {DOC_FREQ_VIEW}").execute(
            f"CREATE VIEW {DOC_FREQ_VIEW} AS "
            "SELECT token, count(*) AS df "
            f"FROM {TOKENS_VIEW} "
            "GROUP BY token "
        ).execute(f"drop view if exists {TOKENS_DOC_FREQ_VIEW}").execute(
            f"CREATE VIEW {TOKENS_DOC_FREQ_VIEW} AS "
    ...
    def prefixes(self):
        self._con.execute(f"drop view if exists {PREFIXES_VIEW}").execute(
            f"create view {PREFIXES_VIEW} as "
    ...
    def matches(self):
        self._con.execute(f"drop view if exists {CANDIDATE_SET_VIEW}").execute(
            f"CREATE VIEW {CANDIDATE_SET_VIEW} AS "
        ...
        ).execute(f"drop view if exists {self._out_table_name}").execute(
            f"create view {self._out_table_name} as "
    ...
    def matches_brute_force(self):
        self._con.execute(f"drop view if exists {self._out_table_name}").execute(
            f"create view {self._out_table_name} as "
            "select r1.rid as rid1, r2.rid as rid2 "
            # ", count(*) as overlap "
    ...
    def clear(self):
        self._con.execute(
            f"drop view if exists {INPUT_TABLE};"
            f"drop view if exists {TOKENS_VIEW};"
            f"drop view if exists {DOC_FREQ_VIEW};"
            f"drop view if exists {TOKENS_DOC_FREQ_VIEW};"
            f"drop view if exists {PREFIXES_VIEW};"
            f"drop view if exists {CANDIDATE_SET_VIEW};"
        )
...
class _JaccardJoin(_JaccardTemplateJoin):
    ...
    def document_frequency(self):
        self._con.execute(f"drop view if exists full_outer_{DOC_FREQ_VIEW}").execute(
            f"create view full_outer_{DOC_FREQ_VIEW} as "
        ...
        ).execute(f"drop view if exists {DOC_FREQ_VIEW}").execute(
            f"create view {DOC_FREQ_VIEW} as "
    ...
    def clear(self):
        self._con.execute(
            f"drop view if exists {INPUT_TABLE};"
            f"drop view if exists {TOKENS_VIEW};"
            f"drop view if exists full_outer_{DOC_FREQ_VIEW};"
            f"drop view if exists {DOC_FREQ_VIEW};"
            f"drop view if exists {TOKENS_DOC_FREQ_VIEW};"
            f"drop view if exists {PREFIXES_VIEW};"
            f"drop view if exists {CANDIDATE_SET_VIEW};"
